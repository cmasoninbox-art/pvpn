// Popup script - Firefox compatible (browser.* API)
...
async function checkTier() {
  try {
    const response = await browser.runtime.sendMessage({ type: 'CHECK_TIER' });
    updateUI(response);
  } catch (e) {
...
function updateUI(data) {
  ...
  if (data.tier) tierVal.textContent = data.tier.toUpperCase();
